...
Private CI artifacts are generated by the `Portable Artifacts` GitHub Actions workflow on manual dispatch or `v*.*.*` tags. The workflow uploads private run artifacts only; it does not publish GitHub Releases or change repository visibility.
