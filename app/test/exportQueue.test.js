...
test("Fast Trim snapshots, retry, duplicate, and result history remain immutable", () => {
  const consent = {
    planSchema: 1,
    confirmationToken: "opaque-plan-token",
  ...
  const fastRequest = request("fast", {
    trim: { startS: 2.35, endS: 5.65, mode: "fastCopy", fastCopyConsent: consent },
  });
  let state = enqueue(createExportQueueState(), fastRequest);
  consent.confirmationToken = "mutated-after-enqueue";
  assert.equal(state.items[0].request.trim.fastCopyConsent.confirmationToken, "opaque-plan-token");
  ...
  state = reduceExportQueue(state, {
    ...
    runId: active.runId,
    outcome: { kind: "failed", message: "stale", trimResult },
  });
  trimResult.videoPacketCount = 999;
  assert.equal(state.items[0].lastOutcome.trimResult.videoPacketCount, 120);

  const retryRequest = {
    ...state.items[0].request,
    outputPath: "/outputs/fast-retry.mp4",
  };
  state = reduceExportQueue(state, {
    type: "retry-prepared",
  ...
  assert.equal(state.items[0].request.trim.fastCopyConsent.confirmationToken, "opaque-plan-token");
  assert.equal(state.items[0].history[0].trimResult.videoPacketCount, 120);

  state = reduceExportQueue(state, {
    type: "duplicate-prepared",
    sourceItemId: state.items[0].id,
    request: { ...state.items[0].request, outputPath: "/outputs/fast-copy.mp4" },
    durationS: 4,
  });
  assert.equal(state.items[1].request.trim.fastCopyConsent.confirmationToken, "opaque-plan-token");
