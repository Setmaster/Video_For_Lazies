...
test("portable export smoke enforces ordered workflow and interaction stage history", async () => {
  ...
  assert.match(raw, /GetForegroundWindow\(\)/);
  assert.match(raw, /GetForegroundWindow\(\) -eq \$handle/);
  ...
  assert.match(raw, /SendInput\(uint inputCount, \[In\] INPUT\[\] inputs, int inputSize\)/);
  assert.match(raw, /KEYEVENTF_EXTENDEDKEY = 0x0001/);
  assert.match(raw, /SendVirtualKey\(IntPtr expectedHandle, ushort virtualKey, bool extended\)/);
  assert.match(raw, /SendInput\(\(uint\)inputs\.Length, inputs, Marshal\.SizeOf\(typeof\(INPUT\)\)\)/);
  assert.match(raw, /downSent != \(uint\)inputs\.Length/);
  assert.match(raw, /Thread\.Sleep\(50\)/);
  ...
  assert.match(wrapperRaw, /WINDOWS_SMOKE_MAX_ATTEMPTS = 3/);
  assert.match(wrapperRaw, /WINDOWS_RETRYABLE_AUTOMATION_ERRORS/);
  assert.match(wrapperRaw, /could not establish stable WebView keyboard focus/);
  assert.match(wrapperRaw, /lost stable WebView keyboard focus/);
  assert.match(wrapperRaw, /retrying the complete smoke in a fresh process and WebView profile/);
  assert.match(wrapperRaw, /retryableAutomationMiss && attempt < WINDOWS_SMOKE_MAX_ATTEMPTS/);
