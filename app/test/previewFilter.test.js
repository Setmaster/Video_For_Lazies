...
test("each adjustment maps onto its CSS filter function", () => {
  assert.equal(buildPreviewColorFilter("0.25", "1", "1"), "brightness(1.250)");
  assert.equal(buildPreviewColorFilter("-0.4", "1", "1"), "brightness(0.600)");
  assert.equal(buildPreviewColorFilter("0", "1.5", "1"), "contrast(1.500)");
  assert.equal(buildPreviewColorFilter("0", "1", "2"), "saturate(2.000)");
  assert.equal(
    buildPreviewColorFilter("0.1", "0.8", "1.3"),
    "brightness(1.100) contrast(0.800) saturate(1.300)",
  );
});

test("filter values clamp to the supported adjustment ranges", () => {
  assert.equal(buildPreviewColorFilter("-2", "1", "1"), "brightness(0.000)");
  assert.equal(buildPreviewColorFilter("5", "1", "1"), "brightness(2.000)");
  assert.equal(buildPreviewColorFilter("0", "9", "1"), "contrast(2.000)");
  assert.equal(buildPreviewColorFilter("0", "1", "9"), "saturate(3.000)");
});
