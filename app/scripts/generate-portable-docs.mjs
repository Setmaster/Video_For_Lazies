...
export function renderThirdPartyNotices({ npmDependencies, cargoDependencies, version, commitSha } = {}) {
  ...
  return [
    ...
    "",
    "## Windows FFmpeg Sidecar",
    "",
    "Windows x64 portable builds bundle a pinned GPL FFmpeg runtime as `ffmpeg-sidecar/`.",
    "",
    ...
    "",
    "The portable Windows folder includes upstream FFmpeg license text, bundle notices, and the source/provenance archives under `ffmpeg-sidecar/source/`.",
    "",
...
export function renderSourceNotice({ version, commitSha, releaseTag, tagAtHead = false } = {}) {
  ...
  return [
    ...
    ...sourceArchiveLines,
    "",
    "## Windows FFmpeg Sidecar Source",
    "",
    "Windows portable builds include a pinned GPL FFmpeg sidecar. The sidecar source and build-provenance archives are included in the portable folder under `ffmpeg-sidecar/source/`.",
    ...
    "",
    "Expected files inside the Windows portable payload:",
    "",
    ...windowsSourceArchiveNames.map((name) => `- \`ffmpeg-sidecar/source/${name}\``),
    "",
  ].join("\n");
}
